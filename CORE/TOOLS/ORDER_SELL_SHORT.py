...
def build_market_order(symbol, side, qty_str, position_side=None, use_reduce_only=False):
    ...
    if use_reduce_only:               # обычно для one-way
        o["reduceOnly"] = "true"
    return o
...
def main():
    ...
    if has_long_short_rows:
        # --- HEDGE MODE: закрываем ТОЛЬКО сторону SHORT (LONG не трогаем) ---
        short_amt = Decimal('0')
        ...
        if short_amt > 0:
            # В hedge закрытие SHORT делаем BUY c positionSide="SHORT"
            orders.append(
                build_market_order(
                    symbol,
                    "BUY",
                    abs_qty_str(short_amt),
                    position_side="SHORT",
                    use_reduce_only=False  # как и раньше, reduceOnly не ставим в hedge
                )
            )

    else:
        # --- ONE-WAY MODE: одна запись BOTH; закрываем ТОЛЬКО если позиция короткая ---
        if positions:
            p = positions[0]
            amt = Decimal(p.get('positionAmt', '0'))
            if amt < 0:
                # Закрываем шорт: покупка с reduceOnly=true
                orders.append(
                    build_market_order(
                        symbol,
                        "BUY",
            ...
            # Если amt >= 0 (лонг или ноль) — ничего не делаем

    # Нет коротких позиций для закрытия — выходим молча (это не ошибка)
    if not orders:
        return
