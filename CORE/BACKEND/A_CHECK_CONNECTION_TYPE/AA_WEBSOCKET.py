...
BEFORE_SCRIPTS = [
    "CORE/BACKEND/A_CHECK_CONNECTION_TYPE/A_CHECK_IF_NEED_RESET/A_run.py",
]

THE_MAIN_SCRIPTS = [
    # ############### Config #########################
    "CORE/TOOLS/CREATE_PERCENT.py",
    "CORE/TOOLS/CREATE_TREND_STATUS.py",
    # ################ RUN ###########################
    "CORE/BACKEND/run_flow.py",
    "CORE/BACKEND/A_CHECK_CONNECTION_TYPE/Z_IF_CANDLE_EMDS/Z_IF_CANDLE_EMDS.py",
    # ################################################
    "CORE/TOOLS/DELAY_BY_SETTINGS.py",
]
        ...
        for script in scripts_list:
            ...
            try:
                ...
                try:
                    ...
                    if description == "основных скриптов":
                        print(f"Прерывание игнорируется для {script_name}, продолжаем выполнение...")
                        continue  # Игнорируем KeyboardInterrupt и переходим к следующему скрипту
                    else:
                        raise  # Для других скриптов прерываем выполнение
                except Exception as e:
                ...
                if description == "основных скриптов":
                    print(f"Прерывание игнорируется для {script}, продолжаем выполнение...")
                    continue  # Игнорируем KeyboardInterrupt на уровне цикла
                else:
                    raise
            except Exception as e:
